import { Flex, VStack, HStack, Heading, Input, Button, Box, Image, Text } from "@chakra-ui/react";
        ...
        <Flex w='100%' justifyContent='center' pt='50px'>
            <VStack w='95%' maxW='500px' alignItems='start' gap='20px'>
                ...
                <VStack w='100%'>
                    {
                        users.map((user) => {
                            return <UserProfile  key={user.username} username={user.username} profile_image={user.profile_image} first_name={user.first_name} last_name={user.last_name} />
                        })
                    }
                </VStack>
            </VStack>
...
const UserProfile = ({username, profile_image, first_name, last_name}) => {
    ...
    return (
        <Flex onClick={handleNav} w='100%' h='100%' border='1px solid' borderColor='gary.300' borderRadius='8px' bg='white' justifyContent='center' alignItems='center'>
